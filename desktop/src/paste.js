...
function pasteText(text, target, ownAppName, options = {}) {
  const preserveClipboard = options.preserveClipboard !== false;
  const restoreDelay = Number.isFinite(options.restoreClipboardDelay)
    ? Math.max(0, options.restoreClipboardDelay)
    : 600;
  const backup = {};
  const textBackup = preserveClipboard ? clipboard.readText() : '';
  const htmlBackup = preserveClipboard ? clipboard.readHTML() : '';
  const rtfBackup = preserveClipboard ? clipboard.readRTF() : '';
  const imageBackup = preserveClipboard ? clipboard.readImage() : null;
