...
function renderLimitBanner() {
  ...
  // Word limits only apply to Base Plan (or if unregistered/guest, since default plan is Base Plan)
  if (profile.plan !== 'Base Plan') return '';
    ...
    return `
      <div class="limit-banner error" style="background: rgba(224, 68, 68, 0.08); border: 1.5px solid rgba(224, 68, 68, 0.2); border-radius: 12px; padding: 14px 18px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; gap: 16px; animation: fadeIn 0.3s ease-out; flex-shrink: 0;">
        <div style="display: flex; align-items: center; gap: 12px;">
          ...
          <div>
            <div style="font-size: 13.5px; font-weight: 700; color: #1a202c; margin-bottom: 2px;">Word limit reached (5,000 words)</div>
      ...
      <div class="limit-banner warning" style="background: rgba(224, 138, 61, 0.08); border: 1.5px solid rgba(224, 138, 61, 0.2); border-radius: 12px; padding: 14px 18px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; gap: 16px; animation: fadeIn 0.3s ease-out; flex-shrink: 0;">
        <div style="display: flex; align-items: center; gap: 12px;">
          ...
          <div>
            ...
            <div style="font-size: 12px; color: #718096; line-height: 1.4;">You have used ${total.toLocaleString()} of your 5,000 monthly words. ${remaining} words remaining.</div>
          </div>
        </div>
        <button class="limit-btn" id="limit-upgrade-btn" style="background: var(--accent); color: white; border: none; padding: 8px 14px; border-radius: 8px; font-size: 12px; font-weight: 700; cursor: pointer; transition: all 0.2s; white-space: nowrap;">Upgrade Plan</button>
      </div>
...
function render() {
  ...
  main.innerHTML = `<div class="view-container">${limitBanner}${viewContent}</div>`;
  ...
  if (profile.registered) {
    ...
    if (planEl) planEl.textContent = profile.plan || 'Base Plan';
  } else {
    if (avatarEl) {
    ...
    if (planEl) planEl.textContent = 'Local Mode';
  }

  // Synchronize pro/upsell card
  const proCard = document.querySelector('.pro-card');
  if (proCard) {
    if (profile.registered && profile.plan === 'Pro Plan') {
      proCard.innerHTML = `
      ...
      `;
    } else if (profile.registered && profile.plan === 'Enterprise Plan') {
      proCard.innerHTML = `
...
async function selectLanguage(code, isSupported) {
  ...
  if (code === 'ml') {
    const profile = state.userProfile || { registered: false, plan: 'Base Plan' };
    const isPro = profile.registered && (profile.plan === 'Pro Plan' || profile.plan === 'Enterprise Plan');
    if (!isPro) {
      lastPasteError = 'Malayalam dictation and translation is a Pro feature. Please register or upgrade your plan in Profile.';
      render();
      openProfileModal();
      return;
    }
...
function renderInsights() {
  ...
  return `
    <div class="insights-header-row">
      <div>
        <div class="ins-title" style="display:flex;align-items:center;gap:8px;">
          Insights 
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" style="width:20px;height:20px;color:var(--accent);"><path d="M12 2L15 9L22 12L15 15L12 22L9 15L2 12L9 9Z" fill="currentColor"/></svg>
        </div>
        <div class="ins-subtitle">Analyze your speech efficiency and dictation metrics.</div>
      </div>
      <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div class="date-selector">
          ...
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="width:12px;height:12px;margin-left:4px;"><polyline points="6 9 12 15 18 9"/></svg>
        </div>
      </div>
    </div>
    ${tabs}
...
function renderStreak(stats) {
  ...
  return `
    <div class="streak-head">
      <h2 class="streak-title" style="display: flex; align-items: center; gap: 6px;">
        <span style="font-size: 20px;">🔥</span> ${stats.streak || 0} day streak
      </h2>
      ...
      <div class="set-card" style="padding: 12px 16px; border-radius: 14px; margin-bottom: 0; display: flex; flex-direction: column; justify-content: space-between; gap: 8px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 6px; width: 100%;">
          <div class="set-info" style="display: flex; flex-direction: column; gap: 2px;">
            ...
            <p class="set-desc" style="font-size: 11px; margin: 2px 0 0; color: var(--muted);">Malayalam translates directly to English.</p>
          </div>
          <div class="set-ctl" style="align-self: flex-start; margin-top: 2px;">
            <div class="seg" style="padding: 3px;">
              ...
              <button class="seg-btn ${state.inputLanguage === 'ml' ? 'seg-active' : ''}" data-lang="ml" style="padding: 5px 10px; font-size: 11.5px;">Malayalam</button>
            </div>
          </div>
        </div>
...
function bindBrainControls() {
  ...
  document.querySelectorAll('[data-use-model]').forEach((btn) => {
    btn.onclick = async () => {
      ...
      if (btn.dataset.useModel === 'medium-q5_0' && !isProOrEnterprise) {
        lastPasteError = "The Medium model is a Pro feature. Please register or upgrade your plan in Profile.";
        render();
        openProfileModal();
        return;
      }
  ...
  document.querySelectorAll('[data-download-model]').forEach((btn) => {
    btn.onclick = () => {
      const profile = state.userProfile || { registered: false, plan: 'Base Plan' };
      const isProOrEnterprise = profile.registered && (profile.plan === 'Pro Plan' || profile.plan === 'Enterprise Plan');
      if (btn.dataset.downloadModel === 'medium-q5_0' && !isProOrEnterprise) {
        lastPasteError = "The Medium model is a Pro feature. Please register or upgrade your plan in Profile.";
        render();
        openProfileModal();
        return;
...
function wireView() {
  ...
  document.querySelectorAll('[data-lang]').forEach((btn) => {
    btn.onclick = async () => {
      const profile = state.userProfile || { registered: false, plan: 'Base Plan' };
      const isProOrEnterprise = profile.registered && (profile.plan === 'Pro Plan' || profile.plan === 'Enterprise Plan');
      if (btn.dataset.lang === 'ml' && !isProOrEnterprise) {
        lastPasteError = "Malayalam dictation and translation is a Pro feature. Please register or upgrade your plan in Profile.";
        render();
        openProfileModal();
        return;
...
window.parayu.onToggleRecording(async (shouldRecord) => {
  ...
  if (shouldRecord && profile.plan === 'Base Plan' && total >= 5000) {
    recording = false;
    window.parayu.notifyRecordingStopped();
    lastPasteError = "Word limit reached (5,000 words). Upgrade your plan to continue using Parayu.";
    render();
    openProfileModal();
    return;
  }
...
function selectPlanCard(plan) {
  ...
  if (salesNote) {
    salesNote.style.display = plan === 'Enterprise Plan' ? 'flex' : 'none';
  }
}
...
function openProfileModal() {
  ...
  if (profile.registered) {
    ...
    selectPlanCard(profile.plan || 'Base Plan');
  } else {
